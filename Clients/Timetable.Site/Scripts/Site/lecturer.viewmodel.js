...
function lecturerViewModel() {
    ...
    self.lecturerSearch = ko.observable();

    self.lecturerSearch.subscribe(function (newValue) {
        if (newValue !== undefined) {
                self.loadLecturersByFilter(newValue, self.currentDepartment().Id);
        } 
    });
    ...
    self.loadLecturersByFilter = function (filter, departmentid) {
        if (filter !== undefined && departmentid !== undefined) {
            self.loadData({
                address: "lecturer/GetByFilter",
                params: {
                    value: filter,
                    departmentid: departmentid
                },
    ...
    self.currentLecturer.subscribe(function (newValue) {
        self.HeadLecturer("");
        if (newValue !== undefined) {
            self.HeadLecturer(newValue.Name);
            self.loadScheduledTutorialsForLecturer();
        }
    });


    self.loadScheduledTutorialsForLecturer = function () {
        ...
        self.loadData({
            ...
            params: {
                curid: 3,
                scheduleinfoid: -1,
                auditoriumid: self.currentLecturer().Id,
                facultyid: -1,
                startdate: "",
                enddate: "",
                weektype: 3,
