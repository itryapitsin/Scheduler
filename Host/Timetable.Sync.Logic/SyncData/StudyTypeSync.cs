...
{
    [Description("Синхронизация форм обучения")]
    public class StudyTypeSync : BaseSync
    {
        public async override void Sync()
        {
            var task1 = Task.Factory.StartNew(() => IIASContext.GetStudyTypes().ToList());
            var task2 = Task.Factory.StartNew(() => SchedulerDatabase.StudyTypes.ToList());
